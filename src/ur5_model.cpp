...
//    : params_{ros::NodeHandle("~")}
{
  ...
  //act_srv->registerGoalCallback();
  //act_srv->registerCancelCallback(boost::bind(&UR5_Model::cancel_action,this));
  ...
  //act_srv = new actionlib::ActionServer<soma_ur5::SOMAFrameworkAction>(*nh,"soma_action",boost::bind(&UR5_Model::execute_action, this,_1,act_srv),false);
  //act_srv->registerGoalCallback(boost::bind(&UR5_Model::execute_action, this,_1,act_srv));
...
void UR5_Model::execute_action(actionlib::ActionServer<soma_ur5::SOMAFrameworkAction>::GoalHandle goal, actionlib::ActionServer<soma_ur5::SOMAFrameworkAction>* as){
  ...
  soma_ur5::SOMAFrameworkFeedback fb;
  ...
  while (goal.getGoalStatus().status==actionlib_msgs::GoalStatus::ACTIVE){
    ros::spinOnce();
    ROS_INFO("YAY,%d %d %d",task,goal.isValid(),goal.getGoalStatus().status);
    fb_pose.position.x=40*(double) std::rand()/RAND_MAX;
    fb.cur_pose=fb_pose;
