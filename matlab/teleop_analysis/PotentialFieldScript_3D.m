...
[x, y, z] = meshgrid(linspace(1,nrows,nrows), linspace(1,nrows,nrows), linspace(1,nrows,nrows));
...
% for i=0.1:0.005:1
k = 0.5;
attractive = k * ( (x - goal(1)).^2 + (y - goal(2)).^2 + (z - goal(3)).^2 );
f = attractive ;

route = GradientBasedPlanner_3D (f, start, goal, 1000);
...
quiver3 (x(yidx,xidx,zidx), y(yidx,xidx,zidx), z(yidx,xidx,zidx),...
        gx(yidx,xidx,zidx),gy(yidx,xidx,zidx), gz(yidx,xidx,zidx), 0.4);
