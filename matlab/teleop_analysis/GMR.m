function [y, Sigma_y, y_tmp, y_tmp2, Sigma_y_tmp, Sigma_y_tmp2,beta_tmp] = GMR(Priors, Mu, Sigma, x, in, out)
%
...
nbData = length(x);%%200 tas
nbVar = size(Mu,1);%% 2 bodiye pas ba ehtesab zaman mishe 3 ta 
nbStates = size(Sigma,3);%% * ta stae darim chun 5 + 3 = 8
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%% Added By Mehrdad %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Heree, variable "in" is 1, since we have only consider time(tempo) as
%%% input variable "out" is [2,3], since the output;s dimention is 2-D of
%%% cartessian coordinat. Here GMM components are K since it composed of K
%%% components.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for i=1:nbStates
  Pxi(:,i) = Priors(i).*gaussPDF(x, Mu(in,i), Sigma(in,in,i));%input Sigma(in,in,i) = Variance for each state
end
beta = Pxi./repmat(sum(Pxi,2)+realmin,1,nbStates);%% It is probably is "sum(h_k)" which is the contribution of eack GMM = {1,...,nbStates}
...
y_tmp2 = repmat(beta_tmp,[length(out) 1 1]) .* y_tmp;% I dont get him why he made it like this ??? equalent to this one: " y_tmp2 = repmat(beta_tmp,[length(out) 1 1]) .* y_tmp; "
y = sum(y_tmp2,3);  % I think what it is doing is to sum up all the 7 states values of y_tmp (i.e. zitta^)
